<template>
  <div
    class="rounded-lg w-full h-full px-2 py-2 shadow-md 
    bg-white
    outline-indigo-400
    dark:outline  
    flex justify-center items-center"
    :class="{ 'outline-none': search !== '' }"
  >
    <UDropdown class="h-full" :items="items" :popper="{ placement: 'bottom-start' }">
      <UButton
        class="shadow-md rounded-lg h-full flex justify-center items-center
         text-white 
         dark:text-white
         text-sm font-bold"
        :trailing-icon="search===''?'i-heroicons-chevron-down-20-solid':''"
      >
        <div class="flex justify-center items-center gap-1">
          <div v-if="current_engine.avatar.url">
            <img
              class="w-4 h-4 rounded-full"
              :src="current_engine.avatar.url"
            />
          </div>
          <div v-else-if="current_engine.avatar.icon">
            <div
              class="w-4 h-4"
              :class="current_engine.avatar.icon"
